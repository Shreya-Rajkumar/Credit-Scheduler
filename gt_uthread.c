...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid, int cred);
...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
   ...
   long int t_diff=0;
	...
	if((u_obj = kthread_runq->cur_uthread))
	{
      ...
      if(u_obj->uthread_tid ==12)
      {
        printf("Time when preempted is %ld s and %ld us \n",u_obj->t1.tv_sec,u_obj->t1.tv_usec);
        printf("Time when scheduled is %ld s and %ld us \n",u_obj->t2.tv_sec,u_obj->t2.tv_usec);
        }
			timersub(&(u_obj->t1), &(u_obj->t2), &(u_obj->t3));
      if(u_obj->uthread_tid ==12)
      printf("Difference is %ld s and %ld us \n",u_obj->t3.tv_sec,u_obj->t3.tv_usec);
      
		  t_diff = u_obj->t3.tv_sec * 1000000L + u_obj->t3.tv_usec;
			ex[u_obj->uthread_tid] = t_diff+ex[u_obj->uthread_tid];
      u_obj->credit = floor(u_obj->credit - (float)(t_diff*25 / KTHREAD_VTALRM_USEC));
      if(u_obj->uthread_tid ==12)
      //printf("Credits to be subtracted for tid 12 is %d",(t_diff*25 / KTHREAD_VTALRM_USEC));
      printf("\nRemaining credits = %d for thread %u with gid = %u \n", u_obj->credit, u_obj->uthread_tid, u_obj->uthread_gid);
		}
