...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	{ 
        kthread_runqueue_t *t;
	runqueue_t *target;
	kthread_context_t *cur, *k;
	cur = kthread_cpu_map[kthread_apic_id()];
   ...
   if(kthread_done==1)
   {
		...
		for(int i = 0; i != cur->cpuid && i < NUM_CPUS; i++)
		{
			k = kthread_cpu_map[i];

			t = &(k->krunqueue);
                        target = t->active_runq;
		
			gt_spin_lock(&(t->kthread_runqlock));
		
			
		
			if(!(target->uthread_mask))
			{
				gt_spin_unlock(&(t->kthread_runqlock));
				break;
			}
			else
			{
				/* Find the highest priority bucket */
				uprio = LOWEST_BIT_SET(target->uthread_mask);
				prioq = &(target->prio_array[uprio]);
				...
				__rem_from_runqueue(target, u_obj);
